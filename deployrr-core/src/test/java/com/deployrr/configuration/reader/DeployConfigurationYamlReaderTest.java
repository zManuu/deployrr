...
public class DeployConfigurationYamlReaderTest {
    ...
    public void testReadSimple() throws IOException {
        try (
                InputStream inputStream = this.getClass().getResourceAsStream("/configuration/simple/deployrr.yml");
                InputStreamReader inputStreamReader = new InputStreamReader(Objects.requireNonNull(inputStream))
        ) {
            DeployConfiguration configuration = this.yamlReader.readConfiguration(inputStreamReader);
